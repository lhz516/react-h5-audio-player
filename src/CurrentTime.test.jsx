...
describe('CurrentTime component', () => {
  ...
  it('renders defaultCurrentTime when no audio provided', () => {
    const { container } = render(
      <CurrentTime defaultCurrentTime={defaultDisplay} isLeftTime={false} timeFormat="auto" />
    )
  ...
  it('displays current playback time (mm:ss) after metadata loaded', () => {
    ...
    const { container } = render(
      <CurrentTime audio={audio} defaultCurrentTime={defaultDisplay} isLeftTime={false} timeFormat="auto" />
    )
    // Simulate browser firing loadedmetadata first
    act(() => {
      audio.dispatch('loadedmetadata')
    })
    expect(container.textContent).toBe('00:30') // ensure explicit expected
  ...
  it('displays remaining (left) time when isLeftTime is true', () => {
    ...
    const { container } = render(
      <CurrentTime audio={audio} defaultCurrentTime={defaultDisplay} isLeftTime={true} timeFormat="mm:ss" />
    )
    act(() => {
      audio.dispatch('loadedmetadata')
    })
  ...
  it('formats as hh:mm:ss when auto and total duration >= 3600', () => {
    ...
    const { container } = render(
      <CurrentTime audio={audio} defaultCurrentTime={defaultDisplay} isLeftTime={true} timeFormat="auto" />
    )
    act(() => {
      audio.dispatch('loadedmetadata')
    })
  ...
  it('honors explicit hh:mm:ss formatting', () => {
    ...
    const { container } = render(
      <CurrentTime audio={audio} defaultCurrentTime={defaultDisplay} isLeftTime={false} timeFormat="hh:mm:ss" />
    )
    act(() => {
      audio.dispatch('loadedmetadata')
    })
